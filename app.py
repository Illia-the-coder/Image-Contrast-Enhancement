...
def solveLinearEquation(IN, wx, wy, lamda):
    ...
    D = 1 - ( dx + dy + dxa + dya)
    A = ((Ax+Ay) + (Ax+Ay).conj().T + scipy.sparse.spdiags(D, 0, k, k)).tocsc()
...
def maxEntropyEnhance(I, isBad, a=-0.3293, b=1.1258):
    ...
    isBad = isBad.astype(np.uint8)  # Convert to uint8
...
def Ying_2017_CAIP(img, mu=0.5, a=-0.3293, b=1.1258):
    ...
    t_b = np.max(I, axis=2)
    t_b_resized = np.array(Image.fromarray(t_b).resize((t_b.shape[1] // 2, t_b.shape[0] // 2), Image.BICUBIC))
    t_our = cv2.resize(tsmooth(t_b_resized, lamda, sigma), (t_b.shape[1], t_b.shape[0]), interpolation=cv2.INTER_AREA)
...
# Gradio app
algorithm_options = ["Ying", "DHE"]
app = gr.Interface(
    fn=process_image,
    inputs=[
        gr.Image(type="numpy", label="Input Image"),
        gr.Radio(algorithm_options, label="Algorithm")
    ],
    outputs=gr.Image(type="numpy", label="Output Image"),
    title="Image Contrast Enhancement",
    description="Apply 'A New Image Contrast Enhancement Algorithm Using Exposure Fusion Framework' or 'A Dynamic Histogram Equalization for Image Contrast Enhancement' to an input image.",
    examples=[
        ["testdata/01.jpg", "Ying"],
        ["testdata/02.jpg", "DHE"]
    ]
)
